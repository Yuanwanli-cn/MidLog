...
def down_sample(logs, labels, sample_ratio):
    ...
    # random_indices = [int(np.random.uniform(0, len(all_index))) for _ in range(sample_num)]
    random_indices = np.random.choice(total_num, size=sample_num, replace=False)
    for i in tqdm(range(sample_num)):
        for key in logs.keys():
            sample_logs[key].append(logs[key][random_indices[i]])
        sample_labels.append(labels[random_indices[i]])
        # del all_index[random_indices[i]]
    return sample_logs, sample_labels


def sliding_window(data_dir, datatype, window_size, data_names, sample_ratio=1):
    '''
    dataset structure
        result_logs(dict):
